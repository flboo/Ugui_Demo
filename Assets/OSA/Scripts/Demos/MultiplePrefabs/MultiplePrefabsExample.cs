	...
	public class MultiplePrefabsExample : OSA<MyParams, BaseVH>, ExpandCollapseOnClick.ISizeChangesHandler	
	{
		...
		protected override BaseVH CreateViewsHolder(int itemIndex)
		{
			...
			if (modelType == typeof(BidirectionalModel)) // very efficient type comparison, since typeof() is evaluated at compile-time
			{
				...
				instance.Init(_Params.bidirectionalPrefab, _Params.Content, itemIndex);
			...
			else if (modelType == typeof(ExpandableModel))
			{
				...
				instance.Init(_Params.expandablePrefab, _Params.Content, itemIndex);
				...
				instance.remoteImageBehaviour.InitializeWithPool(_ImagesPool);
		...
		void ExpandCollapseOnClick.ISizeChangesHandler.OnExpandedStateChanged(RectTransform rt, bool expanded)
		{
			var vh = GetItemViewsHolderIfVisible(rt);

			// If the vh is visible, we update the model's "expanded" field
			if (vh != null)
			{
